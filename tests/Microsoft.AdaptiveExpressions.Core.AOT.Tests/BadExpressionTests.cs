...
namespace Microsoft.AdaptiveExpressions.Core.AOT.Tests
{
    public class BadExpressionTests
    {
        ...
        public void Evaluate(string exp)
        {
            ...
            try
            {
                // Serialize using reflection. This is safe as long as we don't compile with trimming.
#pragma warning disable IL2026, IL3050
                var jsonScope = JsonSerializer.Serialize(scope);
#pragma warning restore IL2026, IL3050
                var (value, error) = Expression.Parse(exp).TryEvaluate(jsonScope);
                if (error != null)
                {
                    passed = true;
                }
