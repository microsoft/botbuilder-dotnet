...
function UpdateLUIS ($lu_file, $appId, $version, $authoringKey, $subscriptionKey, $log)
{
    ...
    # train and publish luis app
        Write-Host "> Training and publishing version ..."
...
function GetActiveVersion($appId, $authoringKey){
    $luisApp = luis get application --appId $appId --authoringKey $authoringKey | ConvertFrom-Json
    return $luisApp.activeVersion
}

function ExportLuFile($appId, $authoringKey, $luFile, $version){
    $luisApp = luis get application --appId $appId --authoringKey $authoringKey | ConvertFrom-Json
    
    if (-not $luFile) {
        $luFile = Join-Path $PSScriptRoot ($luisApp.name + ".lu")
    }
    ...
    $outputFolder = Split-Path $luFile
    $luFileName = Split-Path $luFile -Leaf

    Write-Host "Pulling version" $version "from" $luisApp.name "into" $luFileName
    luis export version --appId $appId --authoringKey $authoringKey --versionId $version | ludown refresh --stdin -o $outputFolder -n $luFileName
}