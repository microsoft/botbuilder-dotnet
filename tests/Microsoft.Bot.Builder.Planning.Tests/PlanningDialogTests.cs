...
namespace Microsoft.Bot.Builder.Planning.Tests
{
    [TestClass]
    public class PlanningDialogTests
    {
        ...
        private TestFlow CreateFlow(PlanningDialog planningDialog, ConversationState convoState, UserState userState)
        {
        ...
        public async Task Planning_TopLevelFallback()
        {
            ...
            var planningDialog = new PlanningDialog("planningTest");

            planningDialog.AddRule(new List<IPlanningRule>()
            {
                new FallbackRule(
                    new List<IDialog>()
                    {
