    ...
    public class TextPromptTests
    {
        ...
        public async Task TextPromptDataValidator()
        {
            ...
            var textPrompt = new TextPrompt()
            {
                ...
                NoMatchResponse = new ActivityTemplate("Make sure the text is greater than three characters."),
            };
            dialogs.Add(textPrompt);
        ...
        [TestMethod]
        public async Task TextPromptDataValidator_Passthrough()
        {
            var convoState = new ConversationState(new MemoryStorage());
            var dialogState = convoState.CreateProperty<DialogState>("dialogState");
            ...
            await new TestFlow(adapter, async (turnContext, cancellationToken) =>
            {
                ...
                {
                    var options = new PromptOptions { };
                    results = await dc.PromptAsync("TextPrompt", options, cancellationToken);
                    if (results.Status == DialogTurnStatus.Complete)
                    {
                        await turnContext.SendActivityAsync(MessageFactory.Text($"Bot already had '{results.Result}'."), cancellationToken);
                    }
                }
                else if (results.Status == DialogTurnStatus.Complete)
                {
            ...
            .Send("hi")
                .AssertReply("Make sure the text is greater than three characters.")
            .Send("bob")
                .AssertReply("Bot received the text 'bob'.")
            .Send("hello")
                .AssertReply("Bot already had 'bob'.")
            .StartTestAsync();
        }
