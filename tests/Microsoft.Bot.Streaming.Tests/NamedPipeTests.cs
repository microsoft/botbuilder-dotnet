...
namespace Microsoft.Bot.Streaming.UnitTests
{
    public class NamedPipeTests
    {
        ...
        public async Task DisconnectWorksAsIntendedAsync()
        {
            ...
            new StreamingRequestHandler(new MockBot(), new Mock<IStreamingActivityProcessor>().Object, pipeName);
            var reader = new NamedPipeClient(pipeName);
            var writer = new NamedPipeServer(pipeName, new StreamingRequestHandler(new MockBot(), new Mock<IStreamingActivityProcessor>().Object, pipeName));
        ...
        {
            var pipeName = Guid.NewGuid().ToString().Substring(0, 18);
            var requestHandler = new StreamingRequestHandler(new MockBot(), new Mock<IStreamingActivityProcessor>().Object, pipeName);
