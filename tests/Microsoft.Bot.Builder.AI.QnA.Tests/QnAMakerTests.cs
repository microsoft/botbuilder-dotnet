...
namespace Microsoft.Bot.Builder.AI.Tests
{
    public class QnAMakerTests
    {
        ...
        public async Task QnaMaker_TraceActivity()
        {
            ...
            TestAdapter adapter = new TestAdapter(TestAdapter.CreateConversation("QnaMaker_TraceActivity"))
        ...
        public async Task QnaMaker_TraceActivity_EmptyText()
        {
            ...
            // No text
            var adapter = new TestAdapter(TestAdapter.CreateConversation("QnaMaker_TraceActivity_EmptyText"));
        ...
        public async Task QnaMaker_TraceActivity_NullText()
        {
            ...
            // No text
            var adapter = new TestAdapter(TestAdapter.CreateConversation("QnaMaker_TraceActivity_NullText"));
            var activity = new Activity
            {
                Type = ActivityTypes.Message,
                Text = null,
        ...
        public async Task QnaMaker_TraceActivity_BadMessage()
        {
            ...
            // No text
            var adapter = new TestAdapter(TestAdapter.CreateConversation("QnaMaker_TraceActivity_BadMessage"));
            var activity = new Activity
            {
        ...
        public async Task QnaMaker_TraceActivity_NullActivity()
        {
            ...
            // No text
            var adapter = new TestAdapter(TestAdapter.CreateConversation("QnaMaker_TraceActivity_NullActivity"));
            var context = new MyTurnContext(adapter, null);

            await Assert.ThrowsAsync<ArgumentNullException>(() => qna.GetAnswersAsync(context));
        }
        ...
        public async Task QnaMaker_Test_UnsuccessfulResponse()
        {
            ...
            await Assert.ThrowsAsync<HttpRequestException>(() => 
                    qna.GetAnswersAsync(GetContext("how do I clean the stove?")));
        }
        ...
        public async Task Telemetry_PII()
        {
            ...
            Assert.NotNull(results);
            Assert.Single(results);
            Assert.StartsWith(results[0].Answer, "BaseCamp: You can use a damp rag to clean around the Power Pack");
            Assert.StartsWith(results[0].Source, "Editorial");
        }
