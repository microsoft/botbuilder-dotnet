...
namespace Microsoft.Bot.Builder.Dialogs
{
    public abstract class DialogCommand : Dialog, IDialogDependencies
    {
        ...
        protected async Task<DialogTurnResult> CancelAllParentDialogsAsync(DialogContext dc, object result = null, CancellationToken cancellationToken = default(CancellationToken), string eventName = "cancelDialog", object eventValue = null)
        {
            if (result is CancellationToken)
            {
                throw new ArgumentException($"{nameof(result)} cannot be a cancellation token");
