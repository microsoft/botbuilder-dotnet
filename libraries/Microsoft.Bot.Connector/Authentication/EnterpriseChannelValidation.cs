...
namespace Microsoft.Bot.Connector.Authentication
{
    public class EnterpriseChannelValidation
    {
        ...
        {
            var channelIssuer = await channelProvider.GetIssuerAsync();

            var parameters =
                 new TokenValidationParameters()
                 {
                     ValidateIssuer = true,
                     ValidIssuers = new[] { channelIssuer},
            ...
            var tokenExtractor = new JwtTokenExtractor(httpClient,
                  parameters,
                  await channelProvider.GetOpenIdMetadataUrlAsync(),
            ...
            Claim audienceClaim = identity.Claims.FirstOrDefault(
                c => c.Issuer == channelIssuer && c.Type == AuthenticationConstants.AudienceClaim);
