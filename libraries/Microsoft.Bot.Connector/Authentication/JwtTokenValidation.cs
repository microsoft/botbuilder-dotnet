    ...
    public static class JwtTokenValidation
    {
        ...
        public static async Task<ClaimsIdentity> ValidateAuthHeader(string authHeader, ICredentialProvider credentials, IChannelProvider channelProvider, string channelId, string serviceUrl = null, HttpClient httpClient = null, AuthenticationConfiguration authConfig = null)
        {
            ...
            else if (channelProvider == null || channelProvider.IsPublicAzure())
            {
                ...
                if (serviceUrl != null)
                {
                    return await ChannelValidation.AuthenticateChannelToken(authHeader, credentials, serviceUrl, httpClient ?? _httpClient, channelId, authConfig);
                }
                else
                {
                    return await ChannelValidation.AuthenticateChannelToken(authHeader, credentials, httpClient ?? _httpClient, channelId, authConfig);
                }
            }
            else if (channelProvider.IsGovernment())
            {
                return await GovernmentChannelValidation.AuthenticateChannelToken(authHeader, credentials, serviceUrl, httpClient ?? _httpClient, channelId, authConfig).ConfigureAwait(false);
            }
            else
            {
                return await EnterpriseChannelValidation.AuthenticateChannelToken(authHeader, credentials, channelProvider, serviceUrl, httpClient ?? _httpClient, channelId, authConfig).ConfigureAwait(false);
            }
        }
    }
}
