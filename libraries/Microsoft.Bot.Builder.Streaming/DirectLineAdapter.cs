...
namespace Microsoft.Bot.Builder.Streaming
{
    ...
    public class DirectLineAdapter : BotFrameworkAdapter, IRequestHandler, IBotFrameworkHttpAdapter
    {
        ...
        /// <param name="onTurnError">Optional function to perform on turn errors.</param>
        /// <param name="bot">The <see cref="IBot"/> to be used for all requests to this handler.</param>
        /// <param name="transportServer">The <see cref="IStreamingTransportServer"/> to be used for streaming transport.</param>
        /// <param name="pipeName">The name of the named pipe to use when creating the server.</param>
        /// <param name="logger">The ILogger implementation this adapter should use.</param>
        public DirectLineAdapter(
            Func<ITurnContext, Exception, Task> onTurnError,
            IBot bot,
            IStreamingTransportServer transportServer = null,
            string pipeName = null,
            ILogger<BotFrameworkHttpAdapter> logger = null)
             : base(new SimpleCredentialProvider())
        {
            ...
            OnTurnError = onTurnError;
            _bot = bot ?? throw new ArgumentNullException(nameof(bot));
            _transportServer = transportServer ?? new NamedPipeServer(pipeName, this);
        ...
        {
            var adapter = new DirectLineAdapter(onTurnError, bot, null, pipeName, logger);
