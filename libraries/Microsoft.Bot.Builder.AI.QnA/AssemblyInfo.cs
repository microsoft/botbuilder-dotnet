// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

using System.Runtime.CompilerServices;

// Allows us to access some internal methods from the Microsoft.Bot.Builder.AI.QnA.Tests unit tests so we don't have to use reflection and we get compile checks.
#if SIGNASSEMBLY
[assembly: InternalsVisibleTo("Microsoft.Bot.Builder.AI.QnA.Tests, PublicKey=")]
#else
[assembly: InternalsVisibleTo("Microsoft.Bot.Builder.AI.QnA.Tests")]
#endif
