...
namespace Microsoft.Bot.Connector.Client.Authentication
{
    ...
    public class AdalAuthenticator : IAuthenticator
    {
        ...
        private readonly ClientAssertionCertificate _clientCertificate;
        private readonly bool _clientCertSendX5C;
        private readonly OAuthConfiguration _authConfig;
        ...
        {
            this._authConfig = configurationOAuth ?? throw new ArgumentNullException(nameof(configurationOAuth));
            this._clientCredential = clientCredential ?? throw new ArgumentNullException(nameof(clientCredential));
            this._logger = logger;
        ...
        {
            this._authConfig = configurationOAuth ?? throw new ArgumentNullException(nameof(configurationOAuth));
            this._clientCertificate = clientCertificate ?? throw new ArgumentNullException(nameof(clientCertificate));
            this._logger = logger;
            this._clientCertSendX5C = sendX5c;
        ...
        private void Initialize(OAuthConfiguration configurationOAuth, HttpClient customHttpClient)
        {
            if (customHttpClient != null)
            {
                ...
                this._authContext = new AuthenticationContext(configurationOAuth.Authority, configurationOAuth.ValidateAuthority, new TokenCache(), httpClientFactory);
            }
            else
            {
                this._authContext = new AuthenticationContext(configurationOAuth.Authority, configurationOAuth.ValidateAuthority);
            }
        }
...
#pragma warning restore VSTHRD103 // Call async methods when in an async method
                ...
                if (acquired)
                {
                    ...
                    {
                        authResult = await _authContext.AcquireTokenAsync(_authConfig.Scope, this._clientCredential).ConfigureAwait(false);
                    }

                    // Certificate based auth
                    else if (_clientCertificate != null)
                    {
                        authResult = await _authContext.AcquireTokenAsync(_authConfig.Scope, _clientCertificate, sendX5c: this._clientCertSendX5C).ConfigureAwait(false);
                    }
