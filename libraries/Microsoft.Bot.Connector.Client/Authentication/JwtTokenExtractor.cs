...
namespace Microsoft.Bot.Connector.Client.Authentication
{
    ...
    internal class JwtTokenExtractor
    {
        ...
        private async Task<ClaimsPrincipal> ValidateTokenAsync(string jwtToken, string channelId, string[] requiredEndorsements)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(keyId) && endorsements.TryGetValue(keyId, out var endorsementsForKey))
                {
                    ...
                    var isEndorsed = ValidateEndorsements(channelId, endorsementsForKey);
                    ...
                    var additionalEndorsementsSatisfied = requiredEndorsements.All(
                            endorsement => ValidateEndorsements(endorsement, endorsementsForKey));
        ...
        /// For example, if an <see cref="Activity"/> comes from WebChat, that activity's
        /// <see cref="Activity.ChannelId"/> property is set to "webchat" and the signing party
        ...
        /// <see cref="Activity.ChannelId"/> property, that to which the Activity is affinitized. Alternatively, it could represent a compliance certification that is required.</param>
        ...
        /// The activity’s <see cref="Activity.ChannelId"/> MUST be found in the endorsement list, or the incoming
        ...
        private static bool ValidateEndorsements(string expectedEndorsement, HashSet<string> endorsements)
        {
