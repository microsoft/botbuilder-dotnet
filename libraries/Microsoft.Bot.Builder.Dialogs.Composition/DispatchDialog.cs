    ...
    public class DispatchDialog : ComponentDialog, IDispatchDialog
    {
        ...
        public override async Task<DialogTurnResult> BeginDialogAsync(DialogContext outerDc, object options = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            if (Routes.TryGetValue(topIntent.intent, out string dialogId))
            {
                ...
                if (dialog == null)
                {
                    throw new ArgumentNullException("dialogId", $"{dialogId} dialog was not found");
                }

                // invoke the subDialog 
                return await dialog.BeginDialogAsync(outerDc, null, cancellationToken);
            }
